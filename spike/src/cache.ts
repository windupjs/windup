...
// SPIKE_CACHE_DIR permite isolar o cache em testes.
export const CACHE_DIR = process.env.SPIKE_CACHE_DIR
  ? path.resolve(process.env.SPIKE_CACHE_DIR)
  : path.resolve(import.meta.dirname, "..", ".cache", "trajetorias");
...
export async function invalidate(entry: CacheEntry): Promise<void> {
  ...
  const staleFile = path.join(CACHE_DIR, `${scenarioId}.stale-${timestamp}.json`);
