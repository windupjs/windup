...
function printRun(metrics: RunMetrics, file: string | null = null): void {
  ...
  console.log(
    `[spike] ${metrics.scenario_id}: ${status} | cache=${metrics.cache} llm_calls=${metrics.llm_calls} ` +
  ...
  if (metrics.failure) {
    console.log(`[spike]   falha: [${metrics.failure.kind}] ação=${metrics.failure.action_id ?? "-"} ${metrics.failure.message}`);
  }
}
...
program
  ...
  .action(async (cenario: string, opts: { cache: boolean; repeat: string }) => {
    ...
    for (let i = 1; i <= repeat; i++) {
      if (repeat > 1) console.log(`[spike] execução ${i}/${repeat}`);
      const metrics = await runScenario(scenario, planner, { useCache: opts.cache });
      printRun(metrics);
      if (metrics.result !== "passed") failures += 1;
    }
    if (repeat > 1) console.log(`[spike] ${repeat - failures}/${repeat} execuções passaram`);
...
cache
  ...
  .action(async () => {
    await clearCache();
    console.log("[spike] cache de trajetórias apagado");
  });
