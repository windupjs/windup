...
| `reporters.ts` | Session reports for CI: JUnit XML, JSON and self-contained HTML (summary stats, per-case status/cache/model/cost, expandable failure + per-action timing via native `<details>`, zero JS) via `run --reporter`, non-zero exit on failure. |
