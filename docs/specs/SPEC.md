...
| `metrics.ts` / `costs.ts` | Every run writes `.windup/runs/<ts>-<scenario>.json` (tokens, calls, model, per-action timing, executed plan, failure classification `network|verification|plan_invalid`). Prices are a dated per-model table; reports recompute from tokens so history stays correct. Scan LLM spend is recorded as `scan-*.json` in the same ledger. `windup costs` aggregates: totals, free replays, scans, per-model, per-scenario, last N. |
