...
**npm package installed as a `devDependency`, executed via `npx rubberduck <cmd>`** (bin in the package). Not a global CLI.

Rationale: version pinned to the project (two projects can use different versions; CI installs the same one as the dev), zero setup friction in pipelines, and the package doubles as a lib (`import { run } from "rubberduck"`) for anyone who wants to integrate with an existing test runner (vitest/jest) — the CLI is a thin shell over the programmatic API. A global CLI may exist later as a convenience (`rubberduck init` before package.json exists), never as the primary form.
...
| Command | Does |
|---|---|
| `rubberduck init` | Creates `rubberduck.config.ts`, `.rubberduck/` (cache gitignore), detects the framework and suggests config; asks the minimum (base_url, LLM provider) |
| `rubberduck scan` | Project indexing: static (code) + optional dynamic (light crawl). Populates the site map |
| `rubberduck scan --update` | Incremental re-indexing: only what changed since the last scan (git diff) |
| `rubberduck run <scenario\|folder>` | Runs scenario(s) — the productized `spike run` |
| `rubberduck fragment extract <scenario> <range>` | Promotes a stretch of a cached plan to a fragment (SPEC-001/E3) |
| `rubberduck status` | Index state: mapped pages, staleness, coverage, cached scenarios |

## Configuration (`rubberduck.config.ts`)
...
Rationale against the watcher/daemon: a resident process is complexity and a source of bugs; the natural moment to update is *before running tests*, not on every save. Recommended integration: automatic `scan --update` at the start of `rubberduck run` (fast, since it is a diff) + optional CI hook. A `--watch` may exist in the future for the DX of scenario authors, as sugar over the incremental mechanism — never as the primary mechanism.

## First-use flow (what the user feels)

```
npm i -D rubberduck
npx rubberduck init          # 3 questions, creates config
npx rubberduck scan          # ~seconds (static); initial project map
# write e2e/scenarios/checkout.json (natural-language task)
npx rubberduck run checkout  # 1st run: plans seeing the map; 2nd onward: replay ~1s
```
...
| P5 | Runner integration (`import` in vitest/jest) | rubberduck scenario running inside a `describe` with the runner's native report |
...
|---|---|---|
| Package name | **Chosen name: Windup** (wind-up toy — you wind it once [LLM plans], it walks on its own [replay]; the wind-up duck keeps the RubberDuck heritage in the logo). Research 2026-07-12: npm `windup` taken by an abandoned CSS framework (7 years — a dispute is plausible); `@windup` org taken; GitHub org `windup` belongs to Red Hat (retired Java migration tool, rebranded MTR — distinct category, low risk) | Plan: package `windupjs` or `@windup-labs/*` + dispute for the bare `windup` in parallel; check domains (`windup.dev`, `getwindup.com`) and an alternative GitHub org (`windup-labs`) before P1. Discarded in this research: rubberduck (taken), rubber-duck, rubberducky |
