...
> **Planning cost, honestly.** A **simple scenario that plans on the first try** is one call: ~3-5s, ~$0.002 on the default model. That is the best case, not the average. **A retry path exists and it can cost 10-30× that** — when a plan comes back invalid, or the model's reply truncates (more likely on element-heavy pages), the planner retries: measured at **3-5 calls, 30-90s and $0.02-$0.09**, and if it never clears validation you pay that for a failure. So a `$0.07` line in `windup costs` is the retry path, not a bug. Windup prints why each extra call happened (`planner retried 2× — truncated response → raising the output budget`), and raises the output budget after a truncation rather than repeating the same request. The cheapest fix is almost always the *task*: **quote the exact text to verify** ("…verify the text 'Popular parking' appears") — Windup then writes that assertion itself, deterministically, in one call.
...
- **Per-scenario determinism (`network` / `clock`).** The [`network`](#configuration-windupconfigts) stubs and [`clock`](#configuration-windupconfigts) freeze that `windup.config.ts` exposes globally can also live on a **single scenario** — scoped to just that run and **merged over the global config, scenario winning**. That's how you test one error state without collateral: `{ "scenario_id": "erro-lista-500", "network": [{ "url": "v1/passports", "status": 500 }] }` forces a 500 on that endpoint **only here** (global stubs still apply; the normal listing scenario is untouched). `clock` merges field-wise (scenario `now`/`timezone` override global). Applied at context launch, never cached — the plan is planned against the stubbed page, so an error-UI assertion is stable. (A scenario with its own `network`/`clock` skips browser prewarming — the prewarmed context only carries the global config.) The error a stub deliberately produces is **excluded from the [`failOn`](#configuration-windupconfigts) gate** — a stubbed `500` won't trip `--fail-on-resource`/`--fail-on-5xx`.
- **Per-scenario `failOn`.** Open a runtime-health exception for **one** scenario instead of blinding the whole suite: `{ "scenario_id": "…", "failOn": { "resourceErrors": false } }`, or a scenario-only `"ignore"`. It's merged over the global `config.failOn` — the boolean gates take the scenario's value when set, and `ignore` lists **concatenate** (global noise + this scenario's). So a URL you only need to silence for one listing stops being an `ignore` entry every other scenario also has to carry.
...
On finish it writes the **scenario file** *and* **caches the recorded plan**, so `npx windup run <id>` replays it immediately at **$0, no LLM**. If the cache later invalidates (a real UI change), it self-heals by re-planning from the task, like any scenario.

- **Selectors are unique-checked at capture.** Each candidate along the anchor ladder (`#id → [data-testid] → [name] → [aria-label] → [placeholder] → clean unique text`) is accepted only if it **uniquely identifies the element on the page** at that moment. Text is used only when it's short, unique, and **carries no dynamic value** (a count/price is skipped, so a cart link never records `"1…R$ 35,00…"`), and it's read from the element's own direct text, not its descendants'. When nothing stable is unique it falls back to a short structural path and **flags the interaction unstable** — Windup then prints those (`⚠ N interaction(s) have no stable anchor …`), the same spots a screen reader struggles with, so you know where to add a `data-testid` or hand-edit before the scenario enters a suite.
- **The task is human-readable even with `--no-llm`.** It's synthesized from the **visible labels** of what you clicked — `Recorded flow: click "Ver ingressos" → fill "Quantidade" → click "Continuar", verifying "Continuar" (ends at /checkout)` — not an opaque "14 interaction(s)", so a self-heal after an invalidation re-plans from a real description of the flow. (With an LLM key it still writes the one-sentence summary; `--no-llm` skips that call.)
- A **typed password never enters the plan**: it's registered to `.env.local` (gitignored) and the action stores a `value_ref`.

It's a local dev tool — interactive and headful, so it needs a **TTY** (not CI). Under an agent/wrapper with no TTY, allocate a PTY: `script -q /dev/null npx windup record`. Flags: `--url <start>` (defaults to `config.baseUrl`), `--id <id>`, `--force`, `--no-llm` (a task is synthesized from the flow's labels instead of the one-call summary).
...
- **Runtime health as a gate (`--fail-on-console` / `--fail-on-resource` / `--fail-on-5xx`).** A scenario can "pass" while the page threw a JS error, failed to load a resource, or got a silent 5xx. Windup observes all of it passively and **records it always** (`RunMetrics.diagnostics`, shown in the console/HTML/JSON reports — informational, like `--a11y`); these flags (or `config.failOn`) turn them into a **failure** (kind `diagnostics`). The two console-noise classes are **separated**: `--fail-on-console` gates **JS** errors (uncaught exceptions, `console.error`, CSP violations) while `--fail-on-resource` gates **sub-resource 4xx loads** (a broken image/font/xhr) — so you can gate JS health without drowning in broken images. Each recorded console error carries its **originating URL** and a `js`/`resource` `kind`. `ignore` (below) silences known noise **by message or URL**, and deliberate `config.network` stubs (e.g. a `{ status: 500 }`) are excluded — so testing an error state never trips the gate.
- **Device emulation (`--device "<preset>"` / `config.device`).** Run at a Playwright device preset — `"iPhone 14"`, `"Pixel 7"`, `"iPad Pro 11"` — applying viewport, user-agent, scale and mobile/touch. **Cached plans are keyed per device**, so mobile and desktop are separate trajectories (a mobile run can't clobber the desktop plan). Mobile emulation needs chromium.
- **Web vitals + performance budgets (`--web-vitals` / `config.budgets`).** Capture the final page's **TTFB / FCP / LCP / DCL / load / CLS** and report them (informational under `--web-vitals`). Set `config.budgets` (`{ lcp_ms: 2500, cls: 0.1, load_ms: 4000 }`) and a breach **fails the scenario** (kind `budget`). Perf timing varies run to run — set budgets with headroom.
...
- **Browser prewarming (on by default; `--no-prewarm` to disable).** In a sequential `run --all`, while a scenario runs Windup pre-creates the **fresh** context + page the *next* scenario needs — so its ~200 ms launch happens off the critical path instead of as dead wait (a scenario's `setup` segment drops to ~0). Isolation is unchanged: every scenario still gets its own clean context. Only sequential runs prewarm — `--concurrency > 1` already overlaps launches across workers.
...
**`run` flags:** `--all` · `--no-cache` · `--no-map` · `--repeat <n>` · `--concurrency <n>` (parallel) · `--retries <n>` (re-run a flake; forbidden never retried) · `--max-wall <seconds>` (suite time budget) · `--bail` (stop on first failure) · `--fail-on-console` · `--fail-on-resource` · `--fail-on-5xx` (runtime health gates) · `--device <preset>` (emulate a device) · `--web-vitals` (capture TTFB/FCP/LCP/CLS) · `--no-prewarm` (disable next-scenario browser prewarming) · `--browser chromium|firefox|webkit` · `--verbose` (planning/execution heartbeat) · `--stream` (NDJSON events) · `--headed` (show the browser) · `--slowmo <ms>` (demo pace) · `--base-url <url>` · `--llm <provider[:model]>` · `--summary` (AI debrief) · `--suggest` (fix hint on failure) · `--reporter junit|json|html` · `--report-file <path>` · `--shard i/n` · `--tag <names>` · `--trace` · `--github` · `--a11y` · `--watch`
