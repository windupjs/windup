...
- **Runtime health as a gate (`--fail-on-console` / `--fail-on-5xx`).** A scenario can "pass" while the page logged a JS error or got a silent 5xx. Windup observes both passively during the run and **records them always** (`RunMetrics.diagnostics`, shown in the console/HTML/JSON reports — informational, like `--a11y`); these flags (or `config.failOn`) turn them into a **failure** (kind `diagnostics`). Deliberate `config.network` stubs (e.g. a `{ status: 500 }`) and URLs in `config.failOn.ignore` are excluded, so testing an error state never trips the gate.
