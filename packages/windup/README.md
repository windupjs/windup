...
> **Planning cost, honestly.** A **simple scenario that plans on the first try** is one call: ~3-5s, ~$0.002 on the default model. That is the best case, not the average. When a plan comes back invalid (or the API degenerates) the planner retries, and a **hard case has been measured at 3-5 calls, 30-90s and $0.02-$0.09** — and if it never clears validation you pay that for a failure. Windup prints why each extra call happened (`planner retried 2× — invalid plan: …`). The cheapest fix is almost always the *task*: **quote the exact text to verify** ("…verify the text 'Popular parking' appears") — Windup then writes that assertion itself, deterministically, with no extra call.
